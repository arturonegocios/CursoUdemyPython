###########################################################################
##### This is a sample script that runs a Dynamic Steel Moment Frame analysis in OpenSees
...
# Create Grid Line Arrays
XGrids = [0, 30.*12, 45.*12]
YGrids = [0, 15.*12, 27.*12]

FloorMass = np.array([0, 535., 525.])/386.2 #in Kips

# Create Main Grid Nodes
for i in range(len(XGrids)):
    for j in range(len(YGrids)):
        OData.CreateNode(XGrids[i], YGrids[j], GridX=i, GridY=j)
...
GeoTransfPDelta = OpenSeesAPI.Model.Element.GeomTransf.PDelta(2)
OData.AddObject(GeoTransfLinear)
OData.AddObject(GeoTransfPDelta)
...
OData.AddObject(OpenSeesAPI.TCL.CodeTitle('Define Materials, Sections and Elements'))
...
def CreateColumn(XGrid, YGrid): #YGrid must be more than 0
    ...
    BaseSubNode = OData.CreateNode(XGrids[XGrid],YGrids[YGrid-1],NodeType=2,GridX=XGrid,GridY=YGrid-1)
    TopNode = OData.GetNodesByGrid(XGrid,YGrid)[0]
    TopSubNode = OData.CreateNode(XGrids[XGrid],YGrids[YGrid],NodeType=2,GridX=XGrid,GridY=YGrid)

    Mat = OData.AddMaterial(OpenSeesAPI.Model.Element.Material.UniaxialMaterial.Bilin(OData.GetFreeMaterialId(1,1), Ks, Alpha, Alpha, Mycol, -1*Mycol, Lambda, Lambda, Lambda, Lambda, 1, 1, 1, 1, ThetaP, ThetaP, ThetaPC, ThetaPC, Kappa, Kappa, 1.0, 1.0, 1.0, 1.0))
    OData.AddElement(OpenSeesAPI.Model.Element.Element.ZeroLength(OData.GetFreeElementId(9,1), BaseNode, BaseSubNode, [Mat], [3]))
    OData.AddElement(OpenSeesAPI.Model.Element.Element.ZeroLength(OData.GetFreeElementId(9,1), TopSubNode, TopNode, [Mat], [3]))
    OData.AddConstraint(OpenSeesAPI.Model.Constraint.EqualDOF(BaseNode,BaseSubNode,[1,2]))
    OData.AddConstraint(OpenSeesAPI.Model.Constraint.EqualDOF(TopNode,TopSubNode,[1,2]))
    OData.AddElement(OpenSeesAPI.Model.Element.Element.ElasticBeamColumn(OData.GetFreeElementId(5,1), BaseSubNode, TopSubNode, Acol, E, Icol, GeoTransfPDelta))
...
def CreateBeam(StartXGrid, EndXGrid, YGrid): #YGrid must be more than 0
    ...
    Ks = 6.0*E*Ibeam/(XGrids[EndXGrid]-XGrids[StartXGrid])*n
    a_mem = (n+1)*(Mybeam*0.05)/(Ks*ThetaP)
    Alpha = a_mem/(1.+n*(1.0-a_mem))

    #Get and Create Sub Nodes for the Zero Length Elements
    StartNode = OData.GetNodesByGrid(StartXGrid,YGrid)[0]
    StartSubNode = OData.CreateNode(XGrids[StartXGrid],YGrids[YGrid],NodeType=2,GridX=StartXGrid,GridY=YGrid)
    EndNode = OData.GetNodesByGrid(EndXGrid,YGrid)[0]
    EndSubNode = OData.CreateNode(XGrids[EndXGrid],YGrids[YGrid],NodeType=2,GridX=EndXGrid,GridY=YGrid)

    Mat = OData.AddMaterial(OpenSeesAPI.Model.Element.Material.UniaxialMaterial.Bilin(OData.GetFreeMaterialId(1,1), Ks, Alpha, Alpha, Mybeam, -1*Mybeam, Lambda, Lambda, Lambda, Lambda, 1, 1, 1, 1, ThetaP, ThetaP, ThetaPC, ThetaPC, Kappa, Kappa, 1.0, 1.0, 1.0, 1.0))
    OData.AddElement(OpenSeesAPI.Model.Element.Element.ZeroLength(OData.GetFreeElementId(9,1), StartNode, StartSubNode, [Mat], [3]))
    OData.AddElement(OpenSeesAPI.Model.Element.Element.ZeroLength(OData.GetFreeElementId(9,1), EndNode, EndSubNode, [Mat], [3]))
    OData.AddConstraint(OpenSeesAPI.Model.Constraint.EqualDOF(StartNode,StartSubNode,[1,2]))
    OData.AddConstraint(OpenSeesAPI.Model.Constraint.EqualDOF(EndNode,EndSubNode,[1,2]))
    OData.AddElement(OpenSeesAPI.Model.Element.Element.ElasticBeamColumn(OData.GetFreeElementId(6,1), StartSubNode,EndSubNode, Abeam, E, Ibeam, GeoTransfPDelta))

CreateBeam(0,1,1)
CreateBeam(0,1,2)

# Add PDelta Column and Truss Element
BasePDNode = OData.GetNodesByGrid(2,0,NodeType=1)[0]
FirstStoryPDNode = OData.GetNodesByGrid(2,1,NodeType=1)[0]
SecondStoryPDNode = OData.GetNodesByGrid(2,2,NodeType=1)[0]

FirstStoryMainStructureNode = OData.GetNodesByGrid(1,1,NodeType=1)[0]
SecondStoryMainStructureNode = OData.GetNodesByGrid(1,2,NodeType=1)[0]

OData.AddElement(OpenSeesAPI.Model.Element.Element.ElasticBeamColumn(OData.GetFreeElementId(8,1),BasePDNode,FirstStoryPDNode,1e6, 1e6, 1e2, GeoTransfPDelta))
OData.AddElement(OpenSeesAPI.Model.Element.Element.ElasticBeamColumn(OData.GetFreeElementId(8,1),FirstStoryPDNode,SecondStoryPDNode,1e6, 1e6, 1e2, GeoTransfPDelta))
Mat = OData.AddMaterial(OpenSeesAPI.Model.Element.Material.UniaxialMaterial.Elastic(OData.GetFreeMaterialId(1,1), 29000.))
OData.AddElement(OpenSeesAPI.Model.Element.Element.Truss(OData.GetFreeElementId(8,1), FirstStoryMainStructureNode, FirstStoryPDNode, 1e6, Mat))
OData.AddElement(OpenSeesAPI.Model.Element.Element.Truss(OData.GetFreeElementId(8,1), SecondStoryMainStructureNode, SecondStoryPDNode, 1e6, Mat))

##############################################################################
### Start Writing Elements to the Executible File
##############################################################################

# Setting Nodes as Used (sometimes you make a lot of nodes which you don't use)
...
#Defining Fixity
BaseNode1 = OData.GetNodesByGrid(0,0,NodeType=1)[0]
BaseNode2 = OData.GetNodesByGrid(1,0,NodeType=1)[0]
BaseNode3 = OData.GetNodesByGrid(2,0,NodeType=1)[0]
OData.AddConstraint(OpenSeesAPI.Model.Constraint.Fix(BaseNode1,[1,1,1]))
OData.AddConstraint(OpenSeesAPI.Model.Constraint.Fix(BaseNode2,[1,1,1]))
OData.AddConstraint(OpenSeesAPI.Model.Constraint.Fix(BaseNode3,[1,1,0]))

#Defining Masses

for j in range(1,len(YGrids)):
    for i in range(len(XGrids)-1):
        for node in OData.GetNodesByGrid(i,j):
            OData.AddObject(OpenSeesAPI.Model.Node.Mass(node, [FloorMass[j]/2., 1.e-6, 1.e-6]))
...
for i in range(1,len(YGrids)):
    Node = OData.GetNodesByGrid(2,i)[0]
    Loads.append(OpenSeesAPI.Model.Pattern.Load(Node,[0, -1*FloorMass[i]*386.2, 0]))
...
Displacement_File_Name = '%s-NodeD.dat'%(ModelName)
OData.AddObject(OpenSeesAPI.Output.Recorder.Node(OutputFolder+'/'+Displacement_File_Name, [SecondStoryMainStructureNode], [1], 'disp'))

Velocity_File_Name = '%s-NodeV.dat'%(ModelName)
OData.AddObject(OpenSeesAPI.Output.Recorder.Node(OutputFolder+'/'+Velocity_File_Name, [SecondStoryMainStructureNode], [1], 'vel'))

Acceleration_File_Name = '%s-NodeA.dat'%(ModelName)
OData.AddObject(OpenSeesAPI.Output.Recorder.Node(OutputFolder+'/'+Acceleration_File_Name, [SecondStoryMainStructureNode], [1], 'accel','-timeSeries %d'%TimeSeries.id))

Reaction_File_Name = '%s-NodeReact.dat'%(ModelName)
OData.AddObject(OpenSeesAPI.Output.Recorder.Node(OutputFolder+'/'+Reaction_File_Name, OData.GetNodesByYGrid(0, NodeType=1), [1], 'reaction'))
...
OData.AddObject(OpenSeesAPI.Analysis.Test.EnergyIncr(1.e-6, 1000))
...
#Analysis Options
OData.AddObject(OpenSeesAPI.Analysis.Constraints.Plain())
OData.AddObject(OpenSeesAPI.Analysis.Numberer.RCM())
OData.AddObject(OpenSeesAPI.Analysis.System.BandGeneral())
OData.AddObject(OpenSeesAPI.Analysis.Test.NormDispIncr(1e-6, 1000))
OData.AddObject(OpenSeesAPI.Analysis.Algorithm.KrylovNewton())
...
try:
    os.remove(TCLFileDirectory+'/%s.out'%(ModelName))
except:
    pass

# os.remove(TCLFileDirectory+'/%s.tcl'%(ModelName))
...
fs = -1*np.sum(Reac[0:DataPoints,1:],axis=1)
u = Displ[0:DataPoints,1]
